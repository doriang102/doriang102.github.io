...
where $$\hat p_K$$ is the estimator for an individual tree and $$\hat p_{RF}$$ is the estimator for the final tree based on averaging over the decision trees. Note that we can also take the mode or median instead of the average here.
